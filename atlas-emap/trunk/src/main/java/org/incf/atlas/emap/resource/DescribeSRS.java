...
public class DescribeSRS extends BaseResouce {
	...
	public DescribeSRSResponseDocument completeResponse() {
		...
		EMAPServiceDAOImpl impl = new EMAPServiceDAOImpl();
		...
		while ( iterator2.hasNext()) {
			...
			vo = (EMAPServiceVO) iterator2.next(); 
...
/*		//Start - Get Slice data
		...
		while (iterator.hasNext()) { 
			...
			sliceElement(	s.addNewSlice(), 1, vo);
		}
*/		
...
*/
	...
	public static void sliceElement(SliceType slice, int identifier, EMAPServiceVO vo){
			...
			if ( vo.getValueDirection().equalsIgnoreCase("front") ) { 
				...
				//(Derived from the coordinate system from Ilya - specific to ABA_REF space only)
				if ( vo.getPlusX().equalsIgnoreCase("down") ) { 
					slice.setXOrientation("ventral");
				} 
				if ( vo.getPlusY().equalsIgnoreCase("left") ) {
