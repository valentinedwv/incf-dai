...
public class Get2DImagesByPOI implements Processlet {
    ...
    public void process(ProcessletInputs in, ProcessletOutputs out, 
		...
		String tolerance = Util.getStringInputValue(in, "tolerance");
		...
		// validate filter (same as above)
		vr = validator.validateNEW("tolerance", tolerance);
		if (!vr.isValid()) {
			if (vr.getDefaultValue() != null) {
				tolerance = vr.getDefaultValue();
			} else {
    			throw new InvalidDataInputValueException(vr.getMessage(), 
    					"tolerance");
			}
	        ...
	        if ( vo.getFromSRSCode().equalsIgnoreCase(abaReference) ||
	        	...
	        	if (paxinosCoordinatesString.startsWith("Error:")) {
	    			...
	    			throw new OWSException( 
	    					"Transformed Coordinates Error: ", paxinosCoordinatesString);
	        	}
	        	...
	        	if (abareferenceCoordinatesString.startsWith("Error:")) {
	        		System.out.println("********************ERROR*********************");
	    			throw new OWSException( 
	    					"Transformed Coordinates Error: ", abareferenceCoordinatesString);
	        	}
	        	//End - exception handling
	        ...
	        if (completeImageList.size() < 0) {
        		System.out.println("********************ERROR*********************");
    			throw new OWSException( 
    					"No images found within " + vo.getTolerance() + " mm.", ControllerException.NO_APPLICABLE_CODE);
	        }
